...
export const fromSmallestXecDenomination = (amount: BigNumber, cashDecimals = appConfig.cashDecimals) => {
  const amountBig = new BigNumber(amount);
  const multiplier = new BigNumber(10 ** (-1 * cashDecimals));
  const amountInBaseUnits = amountBig.times(multiplier);
  return amountInBaseUnits.toNumber();
...
export const getWalletBalanceFromUtxos = (nonSlpUtxos: Utxo_InNode[]) => {
...
export const generateTxOutput = (
  ...
  try {
    ...
    if (isOneToMany && destinationAddressAndValueArray) {
...
export const getXecWalletBalanceFromUtxos = (nonSlpUtxos: Utxo_InNode[]) => {
  const totalBalanceInSatoshis = nonSlpUtxos.reduce(
    (previousBalance, utxo) => previousBalance.plus(new BigNumber(utxo.value)),
    new BigNumber(0)
  );
  return {
    totalBalanceInSatoshis: totalBalanceInSatoshis.toString(),
    totalBalance: fromSmallestXecDenomination(totalBalanceInSatoshis).toString()
  };
};
