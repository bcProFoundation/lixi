...
import { KeyCacheNameBuyOffer, KeyCacheNameOffer, KeyIndexNameBuyOffer, KeyIndexNameOffer } from './escrow.contants';
@Injectable()
export class RedisKeySpaceNotification implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    this.redisKeySpace.on('pmessage', (pattern, channel, message) => {
      ...
      if (message.includes(`lixilotus:timeline:${KeyCacheNameOffer}:`)) {
        const arrKey = message.split(`timeline:${KeyCacheNameOffer}:`);
        const keyFilter = arrKey[arrKey.length - 1];
        this.redis.del(`${KeyIndexNameOffer}:${keyFilter}`);
      }
      if (message.includes(`lixilotus:timeline:${KeyCacheNameBuyOffer}:`)) {
        const arrKey = message.split(`timeline:${KeyCacheNameBuyOffer}:`);
        const keyFilter = arrKey[arrKey.length - 1];
        this.redis.del(`${KeyIndexNameBuyOffer}:${keyFilter}`);
      }
    });
