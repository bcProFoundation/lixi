#!/usr/bin/env bash
#
# Deploy latest code to the lixi production VM without the vault-connected
# build machine.
#
# What it does:
...
#   export CMC_API_KEY=<key> OER_APP_ID=<id>        # only for first fiat-rate deploy
#   ./scripts/deploy-prod.sh [api|web|all] [--yes] [--dry-run]
#
...
#
# Examples:
#   ./scripts/deploy-prod.sh api            # deploy lixi API only (with confirm)
#   ./scripts/deploy-prod.sh all --yes      # deploy API + web without prompting
#   LIXI_REF=cursor/my-fix-3c11 ./scripts/deploy-prod.sh api --dry-run
#
set -euo pipefail
...
if [ -n "${PROD_SSH_KEY:-}" ]; then
  ...
  SCP="scp $SSH_OPTS -i $PROD_SSH_KEY"
elif [ -n "${PROD_SSH_PASS:-}" ]; then
  need sshpass
  SSH="sshpass -p $PROD_SSH_PASS ssh $SSH_OPTS"
  SCP="sshpass -p $PROD_SSH_PASS scp $SSH_OPTS"
else
  echo "Set PROD_SSH_PASS or PROD_SSH_KEY" >&2; exit 1
fi
...
LIXI_REF="${LIXI_REF:-master}"
ECASH_REF="${ECASH_REF:-master}"
...
TAG_SUFFIX="${IMAGE_TAG:-prod-$(date +%Y%m%d-%H%M%S)}"
...
trap 'rm -f "$API_EXTRAS_FILE"' EXIT
for key in CMC_API_KEY OER_APP_ID FIAT_RATE_PROVIDER FIAT_RATE_LEGACY_FALLBACK_ENABLED \
           CMC_CONVERT_LIMIT CMC_COINS FIAT_RATE_HOT_FIATS FIAT_RATE_MERGED_TTL OER_TTL; do
  if [ -n "${!key:-}" ]; then
...
echo "Secrets:           reused from live containers on prod (nothing leaves the VM)"
echo ""
...
if [ "$ASSUME_YES" != "true" ]; then
  read -r -p "Deploy to PRODUCTION ($SERVICES)? [y/N] " answer
  [ "$answer" = "y" ] || [ "$answer" = "Y" ] || { echo "Aborted."; exit 1; }
fi

echo "--- Preflight: prod VM ---"
...
# ---- Remote runner (everything below runs ON the prod VM) ----
...
NEXT_PUBLIC_LIXI_API=https://lixi.social/
NEXT_PUBLIC_LIXI_URL=https://lixi.social/
NEXT_PUBLIC_CHRONIK_URL=https://chronik.pay2stay.com
NEXT_PUBLIC_LIXI_CHRONIK_URL=https://chronik.lixi.app/xec
NEXT_PUBLIC_APPLICATION_URL=https://localecash.com
NEXT_PUBLIC_BOT_USERNAME=LocaleCashBot
NEXT_PUBLIC_ADDRESS_GNC=ecash:pzcpcz67va9ujyk0pt9e5kj975mmdmd25s3gl5avcx
NEXT_PUBLIC_WEB_LINK=https://localecash.com
...
if [ "$WANT_API" = "true" ]; then
  ...
  if health_api; then
    echo "API healthy"
    curl -sf -o /dev/null "https://lixi.social/socket.io/?EIO=4&transport=polling" \
      && echo "external socket.io OK" || echo "WARN: external socket.io check failed"
  else
...
if [ "$WANT_WEB" = "true" ]; then
  ...
  if health_web "$WEB_PORT"; then
    echo "Web healthy"
    curl -sf -o /dev/null https://localecash.com/ \
      && echo "external localecash.com OK" || echo "WARN: external localecash.com check failed"
  else
...
echo "Manual rollback (on prod): docker stop <name> && docker rm <name> && docker rename <name>-prev <name> && docker start <name>"
